...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getAudioData` | `songId` (`PlatformUUID`): The song unique identifier. | [SongAudioData](#devdertypdatasongaudiodata)? | No |  | Get audio analysis data for a song. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `authenticate` | `username` (`String`): The username of the user.<br>`password` (`String`): The password of the user. | [AuthenticationResponse](#devdertypdataauthenticationresponse) | No | IllegalArgumentException, IllegalStateException | Logs in a user and returns JWT token. |
| `refreshToken` | `refreshToken` (`String`): The refresh token. | [AuthenticationResponse](#devdertypdataauthenticationresponse) | No | IllegalArgumentException, IllegalStateException | Refreshes an expired access token. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `listBackups` | - | `List`<[BackupInfo](#devdertypservicesbackupinfo)> | **Yes** | SecurityException | List all available system backup files. |
...
| `createBackup` | - | [BackupResult](#devdertypservicesbackupresult) | **Yes** | SecurityException | Trigger the creation of a full system backup. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `uploadCustomAudio` | `fileData` (`ByteArray`): The raw bytes of the audio file.<br>`fileName` (`String`): The original name of the file.<br>`metadata` ([CustomMetadata](#devdertypdatacustommetadata)?): Optional metadata associated with the audio. | `PlatformUUID`? | No |  | Upload a custom audio file and its metadata. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getSimilarSongs` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs similar to the provided seed tracks. |
| `getSimilarSongsByPlaylist` | `playlistId` (`PlatformUUID`): The user playlist unique identifier to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs similar to the songs in the provided user playlist. |
| `getSimilarSongsByBpm` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs with similar BPM to the provided seed tracks. |
| `getSimilarSongsByEnergy` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs with similar energy to the provided seed tracks. |
| `getSimilarSongsByMood` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs with similar mood (valence) to the provided seed tracks. |
| `getSongsBySameComposers` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs by the same composers as the provided seed tracks. |
| `getSongsBySameLyricists` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs by the same lyricists as the provided seed tracks. |
| `getSongsBySameProducers` | `seedSongIds` (`List`<`PlatformUUID`>): List of song unique identifiers to use as a seed.<br>`limit` (`Int`): The maximum number of results to return. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get songs by the same producers as the provided seed tracks. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getLatestFavSync` | `service` (`SyncServiceType`): The service type (e.g., TIDAL). | [FavSync](#devdertypdatafavsync)? | No |  | Get the timestamp of the last successful synchronization for a specific service. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `handshake` | - | [HandshakeResponse](#devdertypdatahandshakeresponse) | No |  | Perform a handshake test to check server reachability and protocol support. |
...
| `createBatch` | `images` (`List`<[InsertableImage](#devdertypdatainsertableimage)>): Collection of images to store. | `Map`<`String`, `PlatformUUID`> | No |  | Store multiple images on the server in a single operation. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getSyncedLyrics` | `songId` (`PlatformUUID`): The song unique identifier. | [SyncedLyrics](#devdertypservicesmodelssyncedlyrics)? | No |  | Get time-synced lyrics for a song if they exist. |
| `transcribeLyrics` | `songId` (`PlatformUUID`): The song unique identifier.<br>`lyrics` (`String`?): Optional raw lyrics text to transcribe. | [SyncedLyrics](#devdertypservicesmodelssyncedlyrics)? | No | RuntimeException | Trigger AI-based transcription or manual alignment of lyrics. |
...
| `getSongsByUserPlaylist` | `playlistId` (`PlatformUUID`): The playlist unique identifier. | `Flow`<[Song](#devdertypdatasong)> | No | IllegalStateException | Stream all songs belonging to a specific user playlist for mirroring. |
| `getLikedSongs` | `userId` (`PlatformUUID`): The user unique identifier. | `Flow`<[Song](#devdertypdatasong)> | No | IllegalStateException | Stream all songs liked by a specific user for mirroring. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getPlaybackState` | `sessionId` (`PlatformUUID`): The session unique identifier. | [PlaybackState](#devdertypdataplaybackstate)? | No |  | Retrieve the current playback state for a specific session. |
| `setPlaybackState` | `sessionId` (`PlatformUUID`): The session unique identifier.<br>`state` ([PlaybackState](#devdertypdataplaybackstate)): The new playback state data. | `Boolean` | No |  | Update the playback state for a specific session. |
| `observePlaybackState` | `sessionId` (`PlatformUUID`): The session unique identifier. | `Flow`<[PlaybackState](#devdertypdataplaybackstate)> | No |  | Watch real-time playback state changes for a session. |
...
| `getFollowedArtists` | - | `List`<[FollowedArtist](#devdertypservicesmodelsfollowedartist)> | No |  | Get a list of all artists the current user is following. |
| `getRecentReleases` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[RecentRelease](#devdertypservicesmodelsrecentrelease)> | No |  | Retrieve a feed of recent music releases from followed artists. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getRemoteStats` | `config` ([RemoteServerConfig](#devdertypdataremoteserverconfig)): Connection configuration for the remote server. | [ServerStats](#devdertypdataserverstats) | **Yes** | IllegalStateException | Get statistics from a remote Synara instance. |
| `startMirror` | `config` ([RemoteServerConfig](#devdertypdataremoteserverconfig)): Mirroring configuration and credentials. | `Unit` | **Yes** | IllegalStateException | Start mirroring content from another Synara instance. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getGroupedLogs` | - | `Map`<`String`, `List`<[ScheduledTaskLog](#devdertypdatascheduledtasklog)>> | **Yes** | SecurityException | Retrieve a snapshot of the most recent background task logs grouped by task name. |
| `getGroupedLogsFlow` | - | `Flow`<`Map`<`String`, `List`<[ScheduledTaskLog](#devdertypdatascheduledtasklog)>>> | **Yes** | SecurityException | Stream real-time updates for all background task progress and completion. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `getStats` | - | [ServerStats](#devdertypdataserverstats) | No |  | Retrieve detailed system performance metrics and library statistics. |
| `health` | - | `Boolean` | No |  | Simple connectivity check to verify the server is responding. |
| `getProxyInfo` | - | [ProxyInfo](#devdertypdataproxyinfo)? | No |  | Retrieve information about the reverse proxy status and configuration. |
...
| `getSessions` | - | `List`<[Session](#devdertypdatasession)> | No |  | List all sessions for the current user. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `setLiked` | `id` (`PlatformUUID`): The unique UUID of the song.<br>`liked` (`Boolean`): Whether to mark as liked.<br>`addedAt` (`PlatformInstant`?): Optional timestamp of when it was added. | [UserSong](#devdertypdatausersong)? | No |  | Toggle favorite status. |
| `setLyrics` | `id` (`PlatformUUID`): The song unique identifier.<br>`lyrics` (`List`<`String`>): List of lyric lines. | [UserSong](#devdertypdatausersong)? | No |  | Manually set song lyrics. |
| `setArtists` | `id` (`PlatformUUID`): The song unique identifier.<br>`artistIds` (`List`<`PlatformUUID`>): Collection of artist IDs. | [UserSong](#devdertypdatausersong)? | No |  | Update song artists. |
| `setMusicBrainzId` | `id` (`PlatformUUID`): The song unique identifier.<br>`musicBrainzId` (`PlatformUUID`?): The MusicBrainz Recording UUID. | [UserSong](#devdertypdatausersong)? | No |  | Link a song to its MusicBrainz Recording record. |
| `fetchMusicBrainzId` | `id` (`PlatformUUID`): The song unique identifier. | [UserSong](#devdertypdatausersong)? | No |  | Trigger automatic MusicBrainz ID matching for a song. |
| `byId` | `id` (`PlatformUUID`): The song unique identifier. | [UserSong](#devdertypdatausersong)? | No |  | Get song by its unique identifier. |
| `byMusicBrainzId` | `musicBrainzId` (`PlatformUUID`): The MusicBrainz Recording UUID. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Find songs by their MusicBrainz Recording ID. |
| `byIds` | `ids` (`List`<`PlatformUUID`>): Collection of song IDs. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Get multiple songs by their unique identifiers. |
| `byTitle` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`title` (`String`): The song title to search for. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | Search for songs by title. |
| `byArtist` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`artistId` (`PlatformUUID`): The artist unique identifier. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | List songs by a specific artist. |
| `likedByArtist` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`artistId` (`PlatformUUID`): The artist unique identifier.<br>`explicit` (`Boolean`): Whether to include explicit content. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | List songs liked by the user for a specific artist. |
| `byAlbum` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`albumId` (`PlatformUUID`): The album unique identifier. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | List songs in an album. |
| `byPlaylist` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`playlistId` (`PlatformUUID`): The playlist unique identifier. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | List songs in a system playlist. |
| `byUserPlaylist` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`playlistId` (`PlatformUUID`): The user playlist unique identifier. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | List songs in a user playlist. |
| `byOriginalIds` | `ids` (`Collection`<`String`>): Collection of original platform-specific track identifiers. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Find songs by their original platform-specific unique identifiers. |
| `byOriginalTracks` | `tracks` (`Collection`<[Track](#devdertypservicesmetadataimetadataservicetrack)>): Collection of track metadata. | `List`<[UserSong](#devdertypdatausersong)> | No |  | Find songs matching external metadata records. |
| `likedSongs` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`explicit` (`Boolean`): Whether to include explicit content. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | Get all songs liked by the current user. |
| `allSongs` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`explicit` (`Boolean`): Whether to include explicit content.<br>`tags` (`List`<[SongTag](#devdertypdatasongtag)>): Filter by specific tags.<br>`invertTags` (`Boolean`): Invert the tag filter. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | Get all songs with optional filtering. |
| `deleteSongs` | `ids` (`List`<`PlatformUUID`>): Collection of song IDs to delete. | `Boolean` | No |  | Delete multiple songs from the library. |
| `rankedSearch` | `page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`query` (`String`): The search query.<br>`explicit` (`Boolean`): Whether to include explicit content.<br>`liked` (`Boolean`): Only search within liked songs. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserSong](#devdertypdatausersong)> | No |  | Perform a ranked search for songs. |
...
| `downloadSong` | `id` (`PlatformUUID`): The song unique identifier.<br>`quality` (`Int`): Target audio quality.<br>`offset` (`Long`): Byte offset to start from.<br>`chunkSize` (`Int`): Size of each data chunk.<br>`force` (`Boolean`): Whether to force re-transcoding and duration check. | `Flow`<`ByteArray`>? | No | IOException, IllegalStateException | Download song audio in specific quality. |
| `getStreamSize` | `id` (`PlatformUUID`): The song unique identifier. | `Long` | No |  | Get the total size of the song's audio stream. |
| `getDownloadSize` | `id` (`PlatformUUID`): The song unique identifier.<br>`quality` (`Int`): The requested quality.<br>`force` (`Boolean`): Whether to force re-transcoding and duration check. | `Long` | No |  | Get the size of the song audio for a specific quality. |
| `allSongIds` | `explicit` (`Boolean`): Whether to include explicit content.<br>`tags` (`List`<[SongTag](#devdertypdatasongtag)>): Filter by specific tags.<br>`invertTags` (`Boolean`): Invert the tag filter. | `Flow`<`PlatformUUID`> | No |  | Stream all song IDs with optional filtering. |
...
| `listBackups` | - | `List`<[BackupInfo](#devdertypservicesbackupinfo)> | No |  | List all available playlist backup files for the current user. |
...
| `getBackupContent` | `fileName` (`String`): The name of the backup file. | [UserPlaylistBackup](#devdertypdatauserplaylistbackup)? | No |  | Peek into the contents of a specific user playlist backup. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `byId` | `id` (`PlatformUUID`): The playlist unique identifier. | [UserPlaylist](#devdertypdatauserplaylist)? | No |  | Get user playlist by ID. |
| `byIds` | `ids` (`List`<`PlatformUUID`>): Collection of playlist IDs. | `List`<[UserPlaylist](#devdertypdatauserplaylist)> | No |  | Get multiple user playlists by their IDs. |
| `rankedSearch` | `creator` (`PlatformUUID`?): Optional creator ID to filter by.<br>`page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page.<br>`query` (`String`): The search query. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserPlaylist](#devdertypdatauserplaylist)> | No |  | Search user playlists. |
| `allPlaylists` | `creator` (`PlatformUUID`?): Optional creator ID to filter by.<br>`page` (`Int`): Page index.<br>`pageSize` (`Int`): Number of items per page. | [PaginatedResponse](#devdertypdatapaginatedresponse)<[UserPlaylist](#devdertypdatauserplaylist)> | No |  | Get all user playlists. |
| `delete` | `id` (`PlatformUUID`): The playlist unique identifier. | `Boolean` | No |  | Delete a user playlist. |
| `getOrAddPlaylist` | `user` ([User](#devdertypdatauser)): The user who owns the playlist.<br>`customIdentifier` (`String`?): Optional unique string identifier from an external source.<br>`playlist` ([InsertablePlaylist](#devdertypdatainsertableplaylist)): The initial playlist data. | `PlatformUUID` | No |  | Create a new user playlist or retrieve an existing one by a custom identifier. |
...
| `addSongsToPlaylist` | `id` (`PlatformUUID`): The playlist unique identifier.<br>`songIds` (`List`<`PlatformUUID`>): Collection of song IDs to add. | `Unit` | No |  | Add songs to a user playlist. |
| `addAlbumToPlaylist` | `id` (`PlatformUUID`): The playlist unique identifier.<br>`albumId` (`PlatformUUID`): The album unique identifier. | `Unit` | No |  | Add all songs of an album to a user playlist. |
| `addPlaylistToPlaylist` | `id` (`PlatformUUID`): The target playlist unique identifier.<br>`sourcePlaylistId` (`PlatformUUID`): The source playlist unique identifier. | `Unit` | No |  | Add all songs of a playlist to a user playlist. |
| `addUserPlaylistToPlaylist` | `id` (`PlatformUUID`): The target playlist unique identifier.<br>`sourcePlaylistId` (`PlatformUUID`): The source user playlist unique identifier. | `Unit` | No |  | Add all songs of a user playlist to a user playlist. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `findUserById` | `id` (`PlatformUUID`): The unique UUID of the user. | [User](#devdertypdatauser)? | No |  | Look up a user by their unique ID. |
| `findUserByUsername` | `username` (`String`): The username of the user. | [User](#devdertypdatauser)? | No |  | Look up a user by their username. |
| `me` | - | [User](#devdertypdatauser) | No |  | Get the profile of the current authenticated user. |
| `getAllUsers` | - | `List`<[User](#devdertypdatauser)> | **Yes** | IllegalStateException | List all users on the server. |
...
| Function | Parameters | Returns | Admin | Errors | Description |
| :--- | :--- | :--- | :---: | :--- | :--- |
| `logs` | - | `Flow`<[LogLine](#devdertypservicesdownloadlogline)> | No |  | Stream real-time download logs from active processes. |
| `currentDownload` | - | [DownloadQueueEntry](#devdertypservicesdownloaddownloadqueueentry)? | No |  | Get the currently active download task. |
| `downloadQueue` | - | `List`<[DownloadQueueEntry](#devdertypservicesdownloaddownloadqueueentry)> | No |  | Get the list of pending download tasks in the queue. |
| `finishedDownloads` | - | `List`<[FinishedDownloadQueueEntry](#devdertypservicesdownloadfinisheddownloadqueueentry)> | No |  | Get a list of recently completed or failed download tasks. |
...
| `downloadIds` | `ids` (`List`<`PrefixedId`>): Collection of IDs.<br>`type` ([Type](#devdertypservicesdownloadtype)): The type of content (SONG, ALBUM, etc.).<br>`downloader` ([DownloadBackend](#devdertypservicesdownloaddownloadbackend)?): The downloader to use. | `Unit` | No |  | Queue content for download by its IDs. |
| `downloadUrls` | `urls` (`List`<`String`>): Collection of URLs. | `Unit` | No |  | Queue content for download by its URLs. |
| `getDownloaderForUrl` | `url` (`String`): The URL to check. | [DownloadBackend](#devdertypservicesdownloaddownloadbackend)? | No |  | Get the appropriate downloader backend for a given URL. |
| `existsByOriginalId` | `id` (`PrefixedId`): The original ID to check.<br>`type` ([Type](#devdertypservicesdownloadtype)): The type of content. | `Boolean` | No |  | Check if content with a specific original ID is already present in the library. |
| `setDownloadService` | `service` ([DownloadBackend](#devdertypservicesdownloaddownloadbackend)): The downloader service to use. | `Unit` | No |  | Set the preferred downloader backend. |
| `getDownloadService` | - | [DownloadBackend](#devdertypservicesdownloaddownloadbackend) | No |  | Get the currently active downloader backend. |
| `getAllDownloadServices` | - | `List`<[DownloadBackend](#devdertypservicesdownloaddownloadbackend)> | No |  | Get all available downloader backends. |
...
| `search` | `query` (`String`?): General search query.<br>`title` (`String`?): Filter by track title.<br>`artist` (`String`?): Filter by artist name.<br>`count` (`Int`): Maximum number of results. | `List`<[DownloadSong](#devdertypservicesdownloaddownloadsong)> | No | IllegalStateException | Search for tracks directly. |
...
| `getAllMetadataTypes` | `features` (`Set`<`Feature`>): Optional set of features that must be supported by the provider. | `List`<`MetadataType`> | No |  | Get all registered metadata providers, optionally filtered by supported features. |
