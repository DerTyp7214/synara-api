...
- **`IUiService`** (`MockUiService`): serves the server-driven UI fixtures used in [SERVER_DRIVEN_UI.md](SERVER_DRIVEN_UI.md) — a `library` slot tile, the `core.importer` page (toolbar, URL form with scanner portal, live log pane), the modal `core.importer.queue` and `core.importer.settings` pages, the `core.importer.card` home card with pin/unpin state kept in memory, `invoke` handlers that validate the importer form, `intake`/`resolveIntake` (Tidal links and ids → `OK`, Apple Music links → `NEEDS_CHOICE` between Tidal and gamdl, text → the external-search handler; a `resolverId` resolves the choice) and `dispatchHook` returning the same handlers plus "Open in importer". `subscribe` re-emits every two seconds and `subscribeLive("core.importer", "log")` appends a log line every 500 ms so live rendering can be tested.
