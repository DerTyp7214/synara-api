...
private fun registerAuthenticated(koin: Koin, call: ApplicationCall, user: User, registrar: ServiceRegistrar) {
    ...
    registrar.register(IPodcastService::class) { RpcPodcastService(user, koin.get(), koin.get(), koin.get(), koin.get(), koin.get()).withAuthorization<IPodcastService>(user).withLogging<IPodcastService>(call) }
