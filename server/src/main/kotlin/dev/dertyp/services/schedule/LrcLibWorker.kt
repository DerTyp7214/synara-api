...
class LrcLibWorker : KoinComponent {
    private val logger = KtorSimpleLogger("LrcLibWorker")
    ...
    suspend fun run(onProgress: suspend (Double, String) -> Unit = { _, _ -> }): Map<String, Int> {
        if (!isRunning.compareAndSet(expectedValue = false, newValue = true)) {
            logger.info("LrcLibWorker is already running. Skipping this run.")
            return emptyMap()
        }
        ...
        return try {
            logger.info("Starting LrcLibWorker")
            ...
            logger.info("LrcLibWorker finished. Synced: $synced, Not Found: $notFound, Failed: $failed")
